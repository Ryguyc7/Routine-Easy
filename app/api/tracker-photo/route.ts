...
export async function POST(request: Request) {
  ...
  if (!validIdentity(identity) || !(file instanceof File) || !file.type.startsWith("image/") || file.size <= 0 || file.size > 8 * 1024 * 1024) {
  ...
  await runtime.UPLOADS.put(fileKey, file.stream(), { httpMetadata: { contentType: file.type } });
  try {
    await runtime.DB.batch([
      runtime.DB.prepare(`INSERT INTO tracker_entries (owner_key, routine_id, tracker_key, date, value_text, file_key, content_type)
        ...
        .bind(owner, identity.routineId, identity.trackerKey, identity.date, fileKey, file.type),

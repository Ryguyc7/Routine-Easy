...
function normalizeRoutineCollection(items: Routine[]) {
  return items.map((routine, index) => ({
    ...routine,
...
function routineFromForm(form: FormData, id: number, existing: Routine | undefined, nextItemId: number): Routine {
  ...
  return {
    ...
    timeSection: existing?.timeSection ?? cleanRoutineTimeSection(form.get("timeSection")),
...
export default function Home() {
  ...
  function commitRoutineArrangement(next: Routine[]) {
    const ordered = next.map((routine, sortOrder) => ({ ...routine, sortOrder }));
    setRoutines(ordered);
    if (isNativeApp()) return;
    arrangementSaveQueueRef.current = arrangementSaveQueueRef.current.catch(() => undefined).then(async () => {
      const response = await fetch("/api/routines", {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ order: ordered.map(({ id, timeSection, sortOrder }) => ({ id, timeSection, sortOrder })) }),
      });
      if (!response.ok) throw new Error("Routine arrangement failed");
    }).catch(async () => {
      setError("Your routine order could not be saved. Please try again.");
      await loadData();
    });
  }
  ...
  async function loadData() {
    try {
      if (isNativeApp()) {
        ...
        setRoutines(normalizeRoutineCollection(data.routines as Routine[]));
      ...
      setRoutines(normalizeRoutineCollection(data.routines));
  ...
  useEffect(() => {
    ...
    const checkReminders = () => {
      ...
      for (const routine of todayRoutines) {
        ...
        new Notification(`Time for ${routine.name}`, { body: `${routine.emoji} Open Routine EASY to check it off.`, icon: "/routineez-app-icon-light-v1.png", tag: `routine-${routine.id}-${todayKey}` });
        notifiedIds.add(key);
      }
  ...
  async function addRoutine(event: FormEvent<HTMLFormElement>) {
    ...
    const response = await fetch("/api/routines", {
      ...
      body: JSON.stringify({
        ...
        timeSection: form.get("timeSection"),
      }),
    });
    if (response.ok) {
      const data = await response.json();
    ...
    <><main className={`app-shell${nativeApp ? " native-app" : ""}${preferences.motion === "reduced" ? " reduce-motion" : ""}${darkTheme ? " theme-dark" : ""}${tab === "settings" ? " settings-view-open" : ""}`}>
      <aside className="sidebar">
        <div className="brand" aria-label="Routine EASY home">
          <img className="brand-logo" src="/routineez-checklist-v3.png" alt="" />
      ...
      <section className="content sky">
        ...
        <header className="mobile-header">
          ...
          <div className="mobile-wordmark" aria-label="Routine EASY">
            <img src="/routineez-checklist-v3.png" alt="" />
      ...
      {!splashVisible && <nav className={`bottom-nav nav-${bottomNavPhase}${showTemplatePicker || showAdd || editingRoutineId !== null || routineToDelete !== null || historyDayEditorOpen || tab === "settings" ? " creation-flow-hidden" : bottomNavReturning ? " creation-flow-returning" : ""}`} aria-label="Main navigation">
        ...
        <NavButton active={tab === "calendar"} onClick={() => selectBottomTab("calendar")} icon={CalendarDays} label="Calendar" />
  ...
  return createPortal(<div className="feature-dialog-backdrop history-edit-page">
    ...
    <section className="history-edit-dialog" role="dialog" aria-modal="true" aria-labelledby="history-edit-title" style={{ "--history-color": routine.color, "--routine": routine.color } as React.CSSProperties}>
      <div className="history-edit-sticky">
        <header><button type="button" className="app-page-back history-page-back" onClick={onClose} aria-label="Back to history" disabled={Boolean(savingStatus)}><ChevronLeft /></button><span className="history-emoji" aria-hidden="true">{routine.emoji}</span><div><h2 id="history-edit-title">{dateLabel}</h2><small>{routine.name}</small></div></header>
        ...
        {showCorrection && <div className="history-status-editor"><span>Choose status</span><div className="history-edit-options history-edit-options-compact" role="group" aria-label={`Correct status for ${dateLabel}`}>
...
function OnboardingSplash({ leaving = false }: { leaving?: boolean }) {
  return <main className={`onboarding-splash splash-overlay${leaving ? " leaving" : ""}`} aria-label="Loading Routine EASY">
    ...
    <div className="splash-brand">
          <img className="splash-logo" src="/routineez-checklist-v3.png" alt="" />
...
function AddRoutineForm({ template, onSubmit, onCancel, saving, usedEmojis, usedColors }: { template: RoutineTemplate | null; onSubmit: (event: FormEvent<HTMLFormElement>) => void; onCancel: () => void; saving: boolean; usedEmojis: string[]; usedColors: string[] }) {
  ...
  <form ref={formRef} className="add-card add-routine-modal" onSubmit={submitWizard} role="dialog" aria-modal="true" aria-label="Add a routine">
    <input type="hidden" name="timeSection" value={template?.timeSection ?? "auto"} />
...
function RoutineOptionsEditor({ routine, onSubmit, onCancel, saving, usedEmojis, usedColors }: { routine: Routine; onSubmit: (event: FormEvent<HTMLFormElement>) => void; onCancel: () => void; saving: boolean; usedEmojis: string[]; usedColors: string[] }) {
  ...
  <form ref={formRef} className="add-card add-routine-modal edit-routine-wizard" onSubmit={submitWizard} role="dialog" aria-modal="true" aria-label={`Edit ${routine.name}`}>
    <input type="hidden" name="timeSection" value={routine.timeSection} />
    <header className="routine-wizard-header">

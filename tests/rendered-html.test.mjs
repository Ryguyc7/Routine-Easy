...
test("ships the Routine EASY product instead of starter content", async () => {
  ...
  assert.match(settingsCss, /\.settings-view-open \.mobile-header, \.settings-view-open \.bottom-nav \{ display: none; \}/);
...
test("dark mode covers mobile cards and interactive surfaces", async () => {
  ...
  assert.match(css, /html\[data-theme="dark"\] :is\(\.calendar-filter-picker, \.history-filter-picker\) \.filter-pills button:first-child\.active \{[\s\S]*?linear-gradient\(#2b2731,#2b2731\) padding-box,[\s\S]*?box-shadow: 0 0 0 2px rgba\(255,255,255,\.82\)/);
  ...
  assert.match(css, /@keyframes bottom-nav-icon-lift[\s\S]*?translateY\(-27px\)[\s\S]*?animation: bottom-nav-icon-lift \.22s/);
